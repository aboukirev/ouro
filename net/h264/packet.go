...
func (s *NALSink) splitAnnexB(buf []byte) [][]byte {
	units := [][]byte{}
	...
	for off := 0; off < end; {
		if buf[off+2] > 1 {
			...
			if off > prev+1 && buf[off-1] == 0 {
				units = append(units, s.parseEBSP(buf[prev:off-1]))
			} else if off > prev {
				units = append(units, s.parseEBSP(buf[prev:off]))
			}
			off += 3
			prev = off
		} else {
	...
	units = append(units, s.parseEBSP(buf[prev:]))
	return units
}
...
func (s *NALSink) parseEBSP(buf []byte) []byte {
	// TODO: Ideal optimization would be to reuse the buffer and rewrite in-place to minimize allocations.
	// RBSP is always shorter or the same length as EBSP.
	zero := byte(0)
	size := len(buf)
	rbsp := make([]byte, size)
	for off := 0; off < size; {
