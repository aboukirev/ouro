...
func TestParsePPSAmcrest(t *testing.T) {
	data := []byte{
		0xee, 0x3c, 0x30, 0x00,
	}
	params := NewParameterSets()
...
func TestParseSPSAmcrestSprops(t *testing.T) {
	data := []byte{
		0x67, 0x64, 0x00, 0x1f, 0xac, 0x34, 0xc8, 0x05, 0x00, 0x5b, 0xff, 0x01, 0x6e, 0x02, 0x02, 0x02,
		0x80, 0x00, 0x01, 0xf4, 0x00, 0x00, 0x3a, 0x98, 0x74, 0x30, 0x00, 0x4e, 0x2a, 0x00, 0x01, 0x38,
		0xa8, 0x5d, 0xe5, 0xc6, 0x86, 0x00, 0x09, 0xc5, 0x40, 0x00, 0x27, 0x15, 0x0b, 0xbc, 0xb8, 0x50,
		0x00,
	}
	params := NewParameterSets()
	err := params.ParseSprop(data)
	if err != nil {
		t.Error(err)
	}
	sps, ok := params.GetSPS(0)
...
func TestParsePPSAmcrestSprops(t *testing.T) {
	data := []byte{
		0x68, 0xee, 0x3c, 0x30, 0x00,
	}
	params := NewParameterSets()
	err := params.ParseSprop(data)
	if err != nil {
